...
model.add(Dense(output_dim=num_hidden_units, input_dim=784))

## Add rectifier activation function to each neuron
model.add(Activation("relu"))

## Add a fully-connected layer with 128 neurons. The input dim is 784 which is the size of the pixels in one image
model.add(Dense(output_dim=num_hidden_units, input_dim=num_hidden_units))
...
model.compile(loss='categorical_crossentropy', optimizer=SGD(lr=0.0045, momentum=0.95), metrics=["accuracy"])
